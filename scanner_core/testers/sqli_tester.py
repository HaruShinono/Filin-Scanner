...
class SqliTester(BaseTester):
    ...
    def test_form(self, form_data: dict) -> List[Vulnerability]:
        ...
        try:
            ...
            captured_headers = form_data.get('headers', {})  # Nhận Headers xác thực từ Playwright
            ...
            for target_input in inputs:
                ...
                # --- 1. BIỆN PHÁP CHUYÊN DỤNG: AUTH BYPASS ---
                            ...
                            if resp:
                                ...
                                if resp.status_code in [200, 201]:
                                    ...
                                    if any(k in resp.text.lower() for k in
                                        ...
                                        # Gửi payload sai để đối chứng (Đã bổ sung captured_headers)
                ...
                # --- 2. BIỆN PHÁP: BOOLEAN-BASED VERIFICATION ---
    ...
    def _inject_form_payload(self, url, method, inputs, target_param, payload, is_api=False, extra_headers=None):
        ...
        try:
            ...
            # In lỗi chi tiết nếu quá trình gửi request bị crash
