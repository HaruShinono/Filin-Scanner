...
class XssTester(BaseTester):
    ...
    def test_form(self, form_data: dict) -> List[Vulnerability]:
        ...
        captured_headers = form_data.get('headers', {})  # Lấy Auth Headers

        print(f"  [DEBUG-XSS] Fuzzing Attack Point: {method} {url}", flush=True)
    ...
    def _inject_form_payload(self, url, method, inputs, target_param, payload, is_api=False, extra_headers=None):
        ...
        if extra_headers: headers.update(extra_headers)  # Bơm Token vào
